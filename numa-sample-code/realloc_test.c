...
int main(int argc, char **argv)
{
	...
	mem[1]='d';
	printf(" content %c %c \n", mem[0], mem[1]);
	char	*new_mem0 = numa_realloc(mem, page_size, (2)*page_size);
	printf( "new_mem address %p\n", new_mem0);
	new_mem0[1]='e';

	printf(" content %c %c \n", new_mem0[0], new_mem0[1]);
	...
	for (i = 0; i < nr_pages; i++) {
		...
		char	*new_mem = numa_realloc((void *)mem, (i+1)*page_size, (i+2)*page_size);
		if (!new_mem) {
